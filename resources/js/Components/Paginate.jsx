                        ...
                        data.links.map(function (link) {
                            return (link.active || !link.url) ?
                                <span key={link.label} aria-current="page">
                                    <span className="relative rounded-full inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500 dark:text-gray-200 bg-white dark:bg-slate-800 border border-gray-300 dark:border-gray-700 cursor-default leading-5">
                                        {he.decode(link.label)}
                                    </span>
                                </span>
                                :
                                <span key={link.label} aria-current="page">
                                    <a href={link.url} className="relative cursor-pointer rounded-full inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500 dark:text-gray-200 bg-white dark:bg-slate-800 border border-gray-300 dark:border-gray-700 hover:bg-secondary hover:text-white transition-all duration-500 leading-5">
                                        {he.decode(link.label)}
                                    </a>
                                </span>
                        })
